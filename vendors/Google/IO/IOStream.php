...
/**
 * Http Streams based implementation of IO.
 *
 * @author Stuart Langley <slangley@google.com>
 */

class IOStream extends IOAbstract
{
    ...
    public function executeRequest(HttpRequest $request)
    {
        ...
        if ($this->trappedErrorNumber) {
            ...
            throw new IOException(esc_html($error), intval($this->trappedErrorNumber));
        }
        ...
        if (false === $response_data) {
            ...
            $this->client->getLogger()->error('Stream ' . $error);
            throw new IOException(esc_html($error), intval($respHttpCode));
        }
