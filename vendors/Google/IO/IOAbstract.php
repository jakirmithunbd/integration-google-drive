...
use CodeConfig\IGD\Google\Http\HttpCacheParser;
...
abstract class IOAbstract
{
    ...
    {
        $this->client = $client;
        $timeout      = $client->getClassConfig('IOAbstract', 'request_timeout_seconds');
        if ($timeout > 0) {
     ...
     */
    abstract public function executeRequest(HttpRequest $request);
    ...
    public function setCachedRequest(HttpRequest $request)
    {
        ...
        if (HttpCacheParser::isResponseCacheable($request)) {
    ...
    public function makeRequest(HttpRequest $request)
    {
        ...
        if (false !== $cached && $cached instanceof HttpRequest) {
     ...
     * @visible for testing.
     *
     * @return bool|HttpRequest returns the cached object or
     *                          false if the operation was unsuccessful
     */
    public function getCachedRequest(HttpRequest $request)
    {
        if (false === HttpCacheParser::isRequestCacheable($request)) {
            return false;
        }
    ...
    {
        if (HttpCacheParser::mustRevalidate($cached)) {
            $addHeaders = [];
