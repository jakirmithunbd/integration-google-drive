...
class LoggerFile extends LoggerAbstract
{
    ...
    public function __construct(Client $client)
    {
        ...
        $file = $client->getClassConfig('LoggerFile', 'file');
        ...
        $mode = $client->getClassConfig('LoggerFile', 'mode');
        if (!$mode) {
            $this->mode = $mode;
        }

        $this->lock = (bool) $client->getClassConfig('LoggerFile', 'lock');
        $this->file = $file;
    }
    ...
    private function open()
    {
        ...
        if ($this->trappedErrorNumber) {
            throw new LoggerException(
                sprintf(
                    "Logger Error: '%s'",
                    esc_html($this->trappedErrorString ?: 'Unknown error')
                ),
                intval($this->trappedErrorNumber)
            );
        }
