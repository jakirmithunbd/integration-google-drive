...
namespace CodeConfig\IGD\Google\Logger;
...
abstract class LoggerAbstract
{
     ...
     * Default log format
     */
    public const DEFAULT_LOG_FORMAT = "[%datetime%] %level%: %message% %context%\n";
    /**
     * Default date format
     *
    ...
    {
        $this->setLevel(
            $client->getClassConfig('LoggerAbstract', 'level')
        );

        $format          = $client->getClassConfig('LoggerAbstract', 'log_format');
        $this->logFormat = $format ? $format : self::DEFAULT_LOG_FORMAT;

        $format           = $client->getClassConfig('LoggerAbstract', 'date_format');
    ...
    protected function normalizeLevel($level)
    {
        ...
        throw new LoggerException(
            sprintf("Unknown LogLevel: '%s'", esc_html($level) ?: 'null')
        );
    }
