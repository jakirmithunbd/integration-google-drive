...
namespace CodeConfig\IGD\Google;

use CodeConfig\IGD\Google\Auth\AuthAbstract;
use CodeConfig\IGD\Google\Auth\AuthAssertionCredentials;
use CodeConfig\IGD\Google\Auth\AuthException;
use CodeConfig\IGD\Google\Auth\AuthOAuth2;
use CodeConfig\IGD\Google\Cache\CacheAbstract;
use CodeConfig\IGD\Google\Http\HttpBatch;
use CodeConfig\IGD\Google\Http\HttpRequest;
use CodeConfig\IGD\Google\Http\HttpREST;
use CodeConfig\IGD\Google\IO\IOAbstract;
use CodeConfig\IGD\Google\Logger\LoggerAbstract;
...
class Client
{
    ...
    public function __construct($config = null)
    {
        if (is_string($config) && strlen($config)) {
            ...
            if ($this->isAppEngine()) {
                ...
                $config->setCacheClass('CacheMemcache');
            }

            if (version_compare(phpversion(), "5.3.4", "<=") || $this->isAppEngine()) {
                // Automatically disable compress.zlib, as currently unsupported.
                $config->setClassConfig('HttpRequest', 'disable_gzip', true);
            }
        }
        ...
        if ($config->getIoClass() == Config::USE_AUTO_IO_SELECTION) {
            if (function_exists('curl_version') && function_exists('curl_exec')
                                                && !$this->isAppEngine()) {
                $config->setIoClass("IOCurl");
            } else {
                $config->setIoClass("IOStream");
            }
        }
     ...
     * @return AuthAssertionCredentials.
     * @
     */
    public function loadServiceAccountJson($jsonLocation, $scopes)
    {
        ...
        if (isset($data->type) && $data->type == 'service_account') {
            ...
            $cred = new AuthAssertionCredentials(
                $data->client_email,
                $scopes,
                $data->private_key
            );
     ...
     * @return AuthLoginTicket Returns an apiLoginTicket if the verification was
     *                         successful.
     */
    ...
    /**
     * @param $creds AuthAssertionCredentials
     */
     ...
     *
     * @param $request HttpRequest|HttpBatch
     * @throws Exception
    ...
    public function execute($request)
    {
        if ($request instanceof HttpRequest) {
            ...
            if (!$this->getClassConfig("HttpRequest", "disable_gzip")) {
                $request->enableGzip();
            }
            $request->maybeMoveParametersToBody();

            return HttpREST::execute($this, $request);
        } elseif ($request instanceof HttpBatch) {
    ...
    /**
     * @return AuthOAuth2 Authentication implementation
     */
     ...
     * $config->setClassConfig('CacheFile',
